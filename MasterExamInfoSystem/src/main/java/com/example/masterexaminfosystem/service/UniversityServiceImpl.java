...
public class UniversityServiceImpl implements UniversityService {
    ...
    public List<University> findRecommendation(Long uid) {
        List<Long> users = favService.findDistinctUserId();
        Map<Long, List<Long>> favMap = new HashMap<>();
        for (Long user : users) {
            favMap.put(user, favService.findByUserId(user).stream().map(
                    Fav::getUniversityId).collect(Collectors.toList()));
        }
        ...
        int k = 3; //取k相邻个用户的收藏夹
        List<Long> recommendationList = new ArrayList<>(); //利用TreeSet去重
        for (int i = 0; i < k; i++) {
